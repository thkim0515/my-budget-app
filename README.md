...
- 🆕 **상태바 고정 알림 기능 추가** — 설정 > 카드 한도 표시에 "상태바에 남은 한도 고정 알림 표시" 토글 신설. 켜면 상태바에 "현재 남은 한도 : OOO 원"이 고정 알림으로 표시되며, 스와이프로 지워도 다시 나타남(`setOngoing`). 지출이 기록될 때마다 금액이 자동 갱신되고, 토글을 끄면 알림이 즉시 사라지며 다시 생기지 않음
- 🎨 **앱 아이콘 교체** — 기본 템플릿 아이콘(돼지저금통)을 그라디언트 배경 + 화살표·달러 심볼 디자인으로 교체. Adaptive icon(전경/배경 레이어) 및 레거시 아이콘 전체 해상도(mdpi~xxxhdpi) 대응
