...
export default function SyncAction() {
  // [수정] 변수명 일치시킴 (uploadData -> backupManual, downloadAndMerge -> restoreManual)
  const { backupManual, restoreManual } = useSync();
  ...
  const handleUpload = async () => {
    ...
    try {
      // [수정] 올바른 함수명 호출
      const code = await backupManual(password);
  ...
  const handleDownload = async () => {
    ...
    try {
      // [수정] 올바른 함수명 호출
      const success = await restoreManual(password, code);
      if (success) {
